...
Use the sibling checkout if it exists at `../vm0-marketing`; otherwise clone `maxandzoe/okou-marketing` into `/tmp/vm0-marketing`. Find the first `package.json` directory, install dependencies there, sync env from the marketing repo root, and start its `dev` script on port `3042`:
...
if [ -d "$SIBLING_MARKETING_ROOT" ]; then
  ...
  if [ ! -d "$MARKETING_ROOT/.git" ]; then
    ...
    gh repo clone maxandzoe/okou-marketing "$MARKETING_ROOT"
  fi
fi
