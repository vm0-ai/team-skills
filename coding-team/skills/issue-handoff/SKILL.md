...
for i in $(seq 1 $MAX_WORKERS); do
  ...
  ISSUE_COUNT=$(gh issue list --repo maxandzoe/okou --label "$LABEL" --assignee "$ME" --state open --json number --jq 'length')
  PR_COUNT=$(gh pr list --repo maxandzoe/okou --label "$LABEL" --author "$ME" --state open --json number --jq 'length')
