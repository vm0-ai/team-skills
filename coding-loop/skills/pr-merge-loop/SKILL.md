...
1. **Args is a URL** containing `/pull/<number>` or `/issues/<number>` → extract `<number>` (e.g., `https://github.com/okou-ai/okou/pull/6144` → `6144`)
...
3. **PR is still open** (`state` = "OPEN"):
   ...
   - **Not in queue** (empty result, PR is OPEN but not in queue) → PR was ejected:
     ...
     ```bash
     gh api repos/okou-ai/okou/pulls/<PR_NUMBER>/timeline --paginate --jq '.[] | select(.event == "removed_from_merge_queue") | {event, created_at, reason: .reason}' | tail -1
     ```
